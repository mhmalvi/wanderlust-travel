...
function themerex_init_actions() {
    ...
    if (
        THEMEREX_GLOBALS["vc_edit_mode"] &&
        jQuery(".vc_empty-placeholder").length == 0 &&
        THEMEREX_GLOBALS["theme_init_counter"]++ < 30
    ) {
...
function themerex_ready_actions() {
    ...
    jQuery(".menu_main_wrap ul#menu_main")
        .clone()
        .removeAttr("id")
        .removeClass("menu_main_nav")
        .addClass("menu_main_responsive")
        .insertAfter(".menu_main_wrap ul#menu_main");
    ...
    jQuery(".menu_main_wrap .menu_main_responsive li a").on(
        "click",
        function (e) {
            "use strict";
            if (
                jQuery("body").hasClass("responsive_menu") &&
    ...
    themerex_init_sfmenu(
        ".menu_main_wrap ul#menu_main, .menu_user_wrap ul#menu_user"
    );
    ...
    jQuery("#toc, .menu_main_wrap ul li, .menu_user_wrap ul#menu_user li").on(
        "click",
        "a",
        function (e) {
            "use strict";
            ...
            if (jQuery(href.substr(pos)).length > 0) {
                ...
                if (now) {
                    ...
                    themerex_document_set_location(
                        pos == 0 ? loc + href : href
                    );
                    e.preventDefault();
                    return false;
    ...
    jQuery("#viewmore_link").on("click", function (e) {
        "use strict";
        if (
            !THEMEREX_GLOBALS["viewmore_busy"] &&
    ...
    // Change display mode
    jQuery(".woocommerce .mode_buttons a,.woocommerce-page .mode_buttons a").on(
        "click",
        function (e) {
            "use strict";
    ...
    */
    ...
    jQuery(".bookmarks_add").on("click", function (e) {
        ...
        if (!exists) {
            var message_popup = themerex_message_dialog(
                '<label for="bookmark_title">' +
                    THEMEREX_GLOBALS["strings"]["bookmark_title"] +
                    '</label><br><input type="text" id="bookmark_title" name="bookmark_title" value="' +
                    title +
                    '">',
                THEMEREX_GLOBALS["strings"]["bookmark_add"],
                null,
                function (btn, popup) {
                    "use strict";
                    if (btn != 1) return;
                    ...
                    jQuery(".bookmarks_list").append(
                        '<li><a href="' +
                            ...
                            title +
                            '<span class="bookmarks_delete icon-cancel-1" title="' +
                            THEMEREX_GLOBALS["strings"]["bookmark_del"] +
                            '"></span></a></li>'
                    );
                    ...
                    setTimeout(function () {
                        themerex_message_success(
                            THEMEREX_GLOBALS["strings"]["bookmark_added"],
                            THEMEREX_GLOBALS["strings"]["bookmark_add"]
                        );
                    }, THEMEREX_GLOBALS["message_timeout"] / 4);
                }
            );
        } else themerex_message_warning(THEMEREX_GLOBALS["strings"]["bookmark_exists"], THEMEREX_GLOBALS["strings"]["bookmark_add"]);
        e.preventDefault();
        return false;
    });
...
function themerex_scroll_actions() {
    ...
    if (THEMEREX_GLOBALS["top_panel_height"] == 0)
        THEMEREX_GLOBALS["top_panel_height"] =
        ...
        if (jQuery(".top_panel_below .slider_wrap").length > 0) {
            ...
            if (slider_height < 10) {
                slider_height = jQuery(".slider_wrap").hasClass(
                    ...
                    : THEMEREX_GLOBALS["slider_height"];
            }
        }
        if (
            scroll_offset <=
            slider_height + THEMEREX_GLOBALS["top_panel_height"]
        ) {
    ...
    jQuery("#toc .toc_item").each(function () {
        ...
        if (
            ...
            scroll_offset + THEMEREX_GLOBALS["top_panel_height"] < off_next
        )
    ...
    if (themerex_is_responsive_need(THEMEREX_GLOBALS["menu_responsive"])) {
        if (!jQuery("body").hasClass("responsive_menu")) {
            jQuery("body")
                ...
                .addClass("responsive_menu");
            if (jQuery("body").hasClass("menu_relayout"))
                jQuery("body")
                    .removeClass("menu_relayout menu_left")
                    .addClass("menu_right");
        ...
        if (themerex_is_responsive_need(THEMEREX_GLOBALS["menu_relayout"])) {
            if (jQuery("body").hasClass("menu_right")) {
                jQuery("body")
                    .removeClass("menu_right")
                    .addClass("menu_relayout menu_left");
...
function themerex_resize_video_background() {
    ...
    if (
        THEMEREX_GLOBALS["media_elements_enabled"] &&
        ...
        .each(function () {
            ...
            jQuery(this)
                ...
                .superfish({
                    ...
                    speed: THEMEREX_GLOBALS["css_animation"]
                        ? 300
                        : THEMEREX_GLOBALS["menu_slider"]
                        ? 300
                        : 200,
                    speedOut: THEMEREX_GLOBALS["css_animation"]
                        ? 300
                        : THEMEREX_GLOBALS["menu_slider"]
                        ? 300
                        : 200,
...
function themerex_build_page_toc() {
    ...
    if (
        toc_count >
        (THEMEREX_GLOBALS["toc_menu_home"] ? 1 : 0) +
            (THEMEREX_GLOBALS["toc_menu_top"] ? 1 : 0)
    ) {
        if (jQuery("#toc").length > 0) jQuery("#toc .toc_inner").html(toc);
        else
            jQuery("body").append(
                '<div id="toc" class="toc_' +
                    THEMEREX_GLOBALS["toc_menu"] +
                    '"><div class="toc_inner">' +
                    toc +
                    "</div></div>"
            );
...
function themerex_init_isotope() {
    ...
    jQuery(".isotope_wrap:not(.inited)").each(function () {
        "use strict";
        all_images_complete =
            all_images_complete && themerex_check_images_complete(jQuery(this));
    });
    // Wait for images loading
    if (
        !all_images_complete &&
        THEMEREX_GLOBALS["isotope_init_counter"]++ < 30
    ) {
    ...
    jQuery(".isotope_wrap:not(.inited)").each(function () {
        ...
        if (isotope_container.hasClass("alternative")) {
            setTimeout(function () {
                ...
                isotopeResizeGrid(
                    isotope_container,
                    isotope_container.find(".isotope_item")
                );
                // Show elements
                isotope_container
                    .find(".isotope_item")
...
function themerex_init_appended_isotope(posts_container, filters) {
    ...
    if (
        !themerex_check_images_complete(posts_container) &&
        THEMEREX_GLOBALS["isotope_init_counter"]++ < 30
    ) {
        setTimeout(function () {
...
function themerex_init_post_formats() {
    ...
    if (THEMEREX_GLOBALS["popup_engine"] == "pretty") {
        jQuery(
            ...
            "rel",
            "prettyPhoto" +
                (THEMEREX_GLOBALS["popup_gallery"] ? "[slideshow]" : "")
        );
        var images = jQuery(
...
function themerex_init_media_elements(cont) {
    if (
        ...
        if (window.mejs) {
            ...
            cont.find(
                "audio:not(.wp-audio-shortcode),video:not(.wp-video-shortcode)"
            ).each(function () {
...
function themerex_show_system_message() {
    if (THEMEREX_GLOBALS["system_message"]["message"]) {
        if (THEMEREX_GLOBALS["system_message"]["status"] == "success")
            themerex_message_success(
                THEMEREX_GLOBALS["system_message"]["message"],
                THEMEREX_GLOBALS["system_message"]["header"]
            );
            ...
            THEMEREX_GLOBALS["system_message"]["status"] == "error" ||
            THEMEREX_GLOBALS["system_message"]["status"] == "warning"
        )
...
function themerex_toggle_popup(popup) {
    if (popup.css("display") != "none") themerex_hide_popup(popup);
    else themerex_show_popup(popup);
}
    ...
    if (popup.css("display") == "none") {
        if (THEMEREX_GLOBALS["css_animation"])
            popup
                .show()
                .removeClass(
                    "animated fast " + THEMEREX_GLOBALS["menu_animation_out"]
                )
                .addClass(
                    "animated fast " + THEMEREX_GLOBALS["menu_animation_in"]
                );
...
function themerex_registration_validate(form) {
    ...
    if (!error) {
        jQuery
            .post(THEMEREX_GLOBALS["ajax_url"], {
            ...
            .done(function (response) {
                ...
                if (rez.error === "") {
                    result_box
                        ...
                        .html(
                            THEMEREX_GLOBALS["strings"]["registration_success"]
                        );
                    setTimeout(function () {
                    ...
                    result_box
                        ...
                        .html(
                            THEMEREX_GLOBALS["strings"]["registration_failed"] +
                                " " +
                                rez.error
                        );
                }
        ...
        error = themerex_form_validate(form, {
            ...
            rules: [
                ...
                },
                {
                    ...
                    min_length: {
                        value: 1,
                        message: THEMEREX_GLOBALS["strings"]["criteria_empty"],
                    },
                    max_length: {
                        value: 1000,
                        message: THEMEREX_GLOBALS["strings"]["criteria_long"],
                    },
                },
                {
                    field: "date_start",
                    min_length: {
                        value: 1,
                        message:
                            THEMEREX_GLOBALS["strings"]["date_start_empty"],
                    },
                },
                {
                    field: "date_end",
                    min_length: {
                        value: 1,
                        message: THEMEREX_GLOBALS["strings"]["date_end_empty"],
                    },
                },
            ],
        });
            ...
            .done(function (response) {
                ...
                if (rez.error === "") {
                    ...
                    if (form_type == "order")
                        ...
                        result
                            .addClass("sc_infobox_style_success")
                            .html(THEMEREX_GLOBALS["strings"]["send_complete"]);
                } else {
                    result
                        .addClass("sc_infobox_style_error")
                        .html(
                            THEMEREX_GLOBALS["strings"]["send_error"] +
                                " " +
                                rez.error
                        );
                }
    ...
    if (jQuery(window).width() > 480) {
        isotopeItem.each(function () {
            ...
            jQuery(this).css(
                ...
                Math.floor(
                    isotopeWrap.width() /
                        Math.floor(isotopeWrap.width() / isotopeItemWidth)
                ) * w
            );
            jQuery(this).css(
                "height",
                Math.floor(
                    isotopeWrap.width() /
                        Math.floor(isotopeWrap.width() / isotopeItemHeight)
                ) * h
            );
        });
    }
